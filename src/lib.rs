...
fn parse_error_prefixa(attrs: &[syn::Attribute]) -> syn::Result<String> {
    for attr in attrs {
...
fn parse_liaise_variant_attra(attrs: &[syn::Attribute]) -> syn::Result<LiaiseVariantAttr> {
...
fn parse_error_prefix(attrs: &[Attribute]) -> syn::Result<Option<String>> {
    for attr in attrs {
        if !attr.path().is_ident("error_prefix") {
            continue;
        }
...
fn parse_liaise_variant_attr(attrs: &[Attribute]) -> syn::Result<LiaiseVariantAttr> {
    let mut out = LiaiseVariantAttr::default();
        ...
        for meta in items {
            match meta {
                    ...
                    if path.is_ident("msg") || path.is_ident("message") =>
                {
                    ...
                    let s = match lit {
                        Lit::Str(ls) => ls,
                        other => return Err(syn::Error::new_spanned(other, "liaise(msg = \"...\") expected")),
                    };
                    out.msg = Some(s);
                }
...
#[proc_macro_derive(LiaiseCodes, attributes(liaise, error_prefix))]
pub fn derive_liaise_codes(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
    let name = &input.ident;
    ...
    // 3) "ERR"
    let prefix = match parse_liaise_prefix(&input.attrs) {
        Ok(Some(p)) => p,
